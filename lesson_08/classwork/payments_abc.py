...
class Stripe(PaymentSystem):
    ...
    def checkout(self, product: Product, token: UUID) -> UUID:
        order_num: UUID = StripeAPI.checkout(product_name=product.name, price=float(product.price), token=token)
        return order_num
...
class PaymentProcessor:
    def __init__(self, user: User, product: Product, payment_system: PaymentSystem) -> None:
        self.user: User = user
...
if __name__ == "__main__":
    ...
    # Payment system - the class that encapsulates the logic to communicate with provider
